...
# Viper quirk: env vars don't override embedded YAML defaults, so most dev
# overrides go via CLI flags (which DO win). The ONE exception is multiline PEM
# material: Windows argv mangles the embedded newlines in something like
# --providers.incus.tls.clientCert=<PEM>, so tls.X509KeyPair sees garbage and
# fails with "failed to find any PEM data". PEM instead rides an on-disk
# override file that Viper's MergeInConfig layer (conf/setup.go) reads from the
# CWD — YAML literal block scalars (|) preserve the line breaks intact. See
# header comment #6.

# ----------------------------------------------------------------------------
# 9.5 Detect the real Incus daemon (WSL2 distro from Setup-Incus.ps1)
# ----------------------------------------------------------------------------
# When the WSL2 'Incus' distro + client cert are present, write an override
# .lahijan.conf.default.yaml into the repo root so the backend connects to a
# real daemon over HTTPS+mTLS. Otherwise fall back to driver-only mode
# (dashboard renders, no instance ops) and point the operator at Setup-Incus.ps1.
$incusOverrideFile = Join-Path $RepoRoot ".lahijan.conf.default.yaml"
# Always remove a stale override from a prior Run-Dev.ps1 run so -NoIncusRemote
# (or a crashed prior run) does not silently keep remote mode on for the next
# plain `lahijan.exe` launch from the repo root.
if (Test-Path $incusOverrideFile) { Remove-Item $incusOverrideFile -Force }

$incusRemoteWired = $false
...
if (-not $NoIncusRemote) {
    ...
    if ($distros -contains $IncusDistro -and (Test-Path $certFile) -and (Test-Path $keyFile)) {
        # YAML literal block scalars (|) preserve the PEM line breaks that
        # tls.X509KeyPair needs. Indent each line by 8 spaces so the block nests
        # under providers: > incus: > tls: > clientCert:/clientKey:.
        $certPem = (Get-Content $certFile -Raw).Trim()
        $keyPem  = (Get-Content $keyFile -Raw).Trim()
        $pad = "        "  # 8 spaces; matches the clientCert:/clientKey: nesting
        $certBlock = (($certPem -split "`r?`n" | ForEach-Object { $pad + $_ }) -join "`n")
        $keyBlock  = (($keyPem  -split "`r?`n" | ForEach-Object { $pad + $_ }) -join "`n")

        $yaml = @"
# AUTO-GENERATED by scripts/Run-Dev.ps1 - wires the WSL2 Incus daemon.
# Do not edit. gitignored. Removed at the start of the next Run-Dev.ps1 run.
# Only the providers.incus remote keys are set; everything else still comes
# from the embedded default + CLI flags.
providers:
  incus:
    remoteURL: "$IncusRemoteURL"
    tls:
      clientCert: |
$certBlock
      clientKey: |
...
"@
        Set-Content -Path $incusOverrideFile -Value $yaml -NoNewline -Encoding utf8NoBOM

        $incusRemoteWired = $true
        $incusMode = "remote daemon ($IncusRemoteURL) via WSL2 '$IncusDistro'"
        Write-Ok "Real Incus detected (WSL2 '$IncusDistro' + $IncusCertDir) -> wrote $incusOverrideFile"
...
$lahijanArgs = @(
    ...
    # wire up). When the WSL2 daemon + cert are detected above, a gitignored
    # .lahijan.conf.default.yaml in the repo root adds remoteURL + mTLS so
    # create/start/exec hit a real daemon; otherwise the startup Ping against
    # the absent unix socket is a non-fatal warning.
