...
//
// CI guard: fails if website/src/locales/en.json and fa.json have drifted
// out of sync (different key sets, missing values, or wrong types). Mirrors
// the equivalent script in web/scripts/.
...
function readLocale(name) {
  const file = path.join(website, "src", "locales", `${name}.json`);
