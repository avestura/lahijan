...
interface SeoHeadProps {
  ...
  titleKey?: string;
  /** Literal page title, for content that is not translated (docs pages). */
  title?: string;
  /** Translation key for the meta description. */
  descriptionKey?: string;
  /** Literal meta description (docs pages). */
  description?: string;
...
export function SeoHead({
  ...
  const title = rawTitle ?? (titleKey ? t(titleKey) : t("app.name"));
  const siteName = t("app.name");
  const description = rawDescription ?? (descriptionKey ? t(descriptionKey) : t("app.description"));
