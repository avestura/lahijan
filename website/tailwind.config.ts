...
// Mirrors web/tailwind.config.ts so both SPAs share the same design system
// (ADR-0014). Tokens are defined in src/styles/tokens.css and consumed via
// `hsl(var(--token) / <alpha-value>)` so dark-mode flipping is a single
// `class="dark"` swap.
const config: Config = {
  darkMode: ["class"],
